...
static inline void add_attribute (const char *name, void *data,
				  const size_t size);
...
init_ast (void)
{
  ...
  if (NULL ==
      (ttree = (abstract_tree_t *) calloc (1, sizeof (abstract_tree_t))))
...
ast_open_node (void)
{
  ...
  llist_insert_end (tree->current, (void *) node,
		    sizeof (abstract_tree_node_t));
  tree->current = node;
}
 ...
 */
void *
ast_get_attribute (const char *name)
{
 ...
 */
void
ast_add_string (const char *name, const char *str)
{
 ...
 */
void
ast_add_generic (const char *name, void *data, size_t size)
{
 ...
 */
ast_node_t
ast_lookup_symbol (const char *name)
{
...
/*
 * Store a symbol in the symbol table. The symbol that is stored in the table
 ...
 */
void
ast_add_symbol (const char *name)
{
 ...
 */
static inline void
add_attribute (const char *name, void *data, const size_t size)
{
 ...
 */
static inline abstract_tree_node_t *
create_node (void)
{
  ...
  if (NULL ==
      (node =
       (abstract_tree_node_t *) calloc (1, sizeof (abstract_tree_node_t))))
    fatal_error ("cannot create new AST node");

  node->children = create_llist ();
  node->attributes = create_hash_table (37);	// 37 is a prime number of
  // reasonable size.
